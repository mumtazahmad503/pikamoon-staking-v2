    ...
    ReentrancyGuardUpgradeable
{
    ...
    function unstake(uint256 /*_stakeId*/) external {
        ...
        uint256 user_totalWeight = 0;

        // loops over each user stake and adds to the total balance & weight.
        uint256 len = user.stakes.length;
        for (uint256 i; i < len; ) {
            unchecked {
                if (!user.stakes[i].isUnstaked) {
                    Stake.Data storage userStake = user.stakes[i];
                    user_totalWeight += userStake.weight();
                    stakeBalance += user.stakes[i].value;
                }
                ++i;
            }
        }
        ...
        user.userTotalWeight = 0;
        user.rewardsPerWeightPaid = 0;
        // update global states
        globalStakeWeight -= user_totalWeight;
        totalTokenStaked -= stakeBalance;

        // return user stakes
        IPikaMoon(poolToken).safeTransfer(msg.sender, stakeBalance);
        // return user rewards
        IPoolController(poolController).transferRewardTokens(
            rewardToken,
            msg.sender,
    ...
    function claimRewards(
        ...
        if (_restakeLeftOver && _claimPercentage < multiplier) {
            if (poolToken == rewardToken) {
                ...
                uint256 pendingRewardsToClaim = user.pendingRewards;
                user.pendingRewards = 0;
                address poolAddress = IPoolController(poolController).pools(
                    rewardToken
                );
                ICorePool(poolAddress).stakeAsPool(
                    msg.sender,
                    pendingRewardsToClaim
                );
            }
    ...
    function _now256() internal view virtual returns (uint256) {
        // return block timestamp when staking stops
        // for example Thu Feb 27 2025 13:41:43 GMT+0000
        return 1740746503;
    }
